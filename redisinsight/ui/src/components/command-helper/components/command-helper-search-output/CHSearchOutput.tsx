...
import { COMMAND_HELPER_LABELS } from 'uiSrc/components/command-helper/command-helper.labels'
...
const CHSearchOutput = ({ searchedCommands }: Props) => {
  ...
  const renderDescription = (command: string) => {
    ...
    if (args.length) {
      const argString = generateArgsNames(commandSpec.provider, args).join(' ')
      return (
        <Text
          size="s"
    ...
    <>
      {searchedCommands.length > 0 && (
        <div style={{ width: '100%' }}>
          {searchedCommands.map((command: string) => {
            const description = renderDescription(command)
            if (!description) {
              return null
            }
      ...
      {searchedCommands.length === 0 && (
        <div className={styles.defaultScreen}>
          <Text data-testid="search-cmds-no-results">
            {COMMAND_HELPER_LABELS.noResults}
          </Text>
        </div>
      )}
    </>
