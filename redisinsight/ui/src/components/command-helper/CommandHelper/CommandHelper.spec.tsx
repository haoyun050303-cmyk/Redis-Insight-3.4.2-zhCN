...
describe('CliHelper', () => {
  ...
  it('Usage text component should be in the Document when Command is matched and usage exists', () => {
    const { queryByTestId } = render(
      <CommandHelper
        {...instance(mockedProps)}
        commandLine={commandLine}
    ...
    const cliHelperUsage = queryByTestId('cli-helper-usage')

    expect(cliHelperUsage).toBeInTheDocument()
    expect(cliHelperUsage).toHaveTextContent('key：键名。')
  })

  it('Examples text component should be in the Document when Command is matched and examples exist', () => {
    const { queryByTestId } = render(
      <CommandHelper
        {...instance(mockedProps)}
        commandLine={commandLine}
    ...
    const cliHelperExamples = queryByTestId('cli-helper-examples')

    expect(cliHelperExamples).toBeInTheDocument()
    expect(cliHelperExamples).toHaveTextContent('读取字符串键的值。')
    expect(cliHelperExamples).toHaveTextContent('GET key:1')
  })
