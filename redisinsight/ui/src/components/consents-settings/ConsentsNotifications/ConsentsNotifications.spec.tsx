...
describe('ConsentsNotifications', () => {
  ...
  it('should render localized notifications title', () => {
    render(<ConsentsNotifications />)

    expect(screen.getByText('通知')).toBeInTheDocument()
    expect(screen.getByText('显示通知')).toBeInTheDocument()
    expect(
      screen.getByText('选择后会显示通知。否则，通知会显示在通知中心。'),
    ).toBeInTheDocument()
  })
