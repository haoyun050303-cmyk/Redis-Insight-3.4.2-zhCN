...
describe('BulkActions', () => {
  ...
  it('placeholder should render', () => {
    ...
    expect(screen.getByText('删除键')).toBeInTheDocument()
    expect(screen.getByText('上传数据')).toBeInTheDocument()
    expect(screen.getByText('未设置模式或键类型')).toBeInTheDocument()
    expect(
      screen.getByText('要执行批量操作，请先设置匹配模式或选择键类型'),
    ).toBeInTheDocument()
  ...
  it('should call proper event after switch tab', async () => {
    ...
    fireEvent.mouseDown(screen.getByText('上传数据'))
