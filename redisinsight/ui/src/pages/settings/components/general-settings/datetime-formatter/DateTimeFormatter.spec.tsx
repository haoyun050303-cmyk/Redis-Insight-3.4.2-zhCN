...
describe('DateTimeFormatter', () => {
  ...
  it('should render localized date time formatter copy', () => {
    render(<DateTimeFormatter />)

    expect(screen.getByText('日期和时间格式')).toBeInTheDocument()
    expect(
      screen.getByText('指定 Redis Insight 使用的日期和时间格式：'),
    ).toBeInTheDocument()
    expect(
      screen.getByText('指定 Redis Insight 使用的时区：'),
    ).toBeInTheDocument()
  ...
  it('should not show custom btn and input unless custom radio btn is clicked ', async () => {
    ...
    expect(screen.getByText('预设格式')).toBeInTheDocument()
    expect(
  ...
  it('should display invalid format when wrong format is typed in a custom input', async () => {
    ...
    await act(() => fireEvent.click(screen.getByText('自定义')))
    ...
    expect(getByDisplayValue('无效格式')).toBeInTheDocument()
  })

  it('should call proper telemetry events', async () => {
    render(<DateTimeFormatter />)

    await act(async () => fireEvent.click(screen.getByText('自定义')))
    const customInput: Nullable<HTMLElement> = screen.getByTestId(
      'custom-datetime-input',
    )
