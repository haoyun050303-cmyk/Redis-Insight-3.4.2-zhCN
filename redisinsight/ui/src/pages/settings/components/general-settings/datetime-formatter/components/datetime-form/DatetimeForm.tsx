...
const DatetimeForm = ({ onFormatChange }: Props) => {
  ...
  const submitForm = async (values: InitialValuesType) => {
    if (checkDateTimeFormat(values.format).valid) {
      ...
      setError(SETTINGS_LABELS.dateTime.unsupportedFormat)
      formik.setSubmitting(false)
    }
  }
  ...
  const handleFormatCheck = (format = formik.values.format) => {
    ...
    if (format.length > 50) {
      setError(SETTINGS_LABELS.dateTime.formatTooLong)
    } else if (!valid) {
      setError(errorMsg || SETTINGS_LABELS.dateTime.unsupportedFormat)
      onFormatChange?.(SETTINGS_LABELS.dateTime.invalidFormat)
    } else {
      setError('')
  ...
  const dateTimeFormatOptions = [
    {
      ...
      label: SETTINGS_LABELS.dateTime.preselectedFormats,
    },
    {
      value: DatetimeRadioOption.Custom,
      label: SETTINGS_LABELS.dateTime.custom,
    },
  ]
        ...
        {formik.values.selectedRadioOption === DatetimeRadioOption.Custom && (
          <>
            ...
            <RiTooltip
              ...
              content={
                showError
                  ? error || SETTINGS_LABELS.dateTime.unsupportedFormat
                  : null
              }
            >
              <PrimaryButton
                aria-label={SETTINGS_LABELS.dateTime.save}
