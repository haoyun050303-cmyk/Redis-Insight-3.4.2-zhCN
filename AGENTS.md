...
## Local Chinese Build Notes

This workspace contains a local Chinese UI customization. Before upgrading the official RedisInsight source or packaging a Windows build, read `docs/chinese-localization-maintenance.md`.

Important local rules:

- Keep official source history intact; do not make Chinese localization changes directly on `main`.
- Reapply localization from a Chinese feature branch after official upgrades.
- Verify Command Helper still contains `GEOSEARCH` before packaging.
- Package only after the user has manually checked the UI.
